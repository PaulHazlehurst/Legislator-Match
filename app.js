...
function renderImportReview() {
  ...
  const billsHtml = importState.bills.map((b, i) => {
    ...
    return `
    <div class="ibr-card" data-bill-idx="${i}" data-needs-review="${needsReview ? '1' : '0'}" style="border-left:3px solid ${borderColor};">
      <div class="ibr-top">
      ...
      <div class="ibr-fields">
        <div class="ibr-field grow">
          <label class="ibr-label">Topic</label>
          <select class="ibr-select import-topic-select">${topicOptionsHtml(topicCode)}</select>
          <input type="text" class="ibr-input ibr-new-input import-topic-new" placeholder="New topic name" value="${escapeHtml(b.suggestedTopicLabel || '')}" style="display:${topicCode === '__new__' ? 'block' : 'none'};" />
        </div>
        <div class="ibr-field grow">
          <label class="ibr-label">Subtopic</label>
          <select class="ibr-select import-subtopic-select">${subtopicOptionsHtml(topicCode, subtopicCode)}</select>
          <input type="text" class="ibr-input ibr-new-input import-subtopic-new" placeholder="New subtopic" value="${escapeHtml(b.suggestedSubtopicLabel || '')}" style="display:${subtopicCode === '__new__' ? 'block' : 'none'};" />
        </div>
        <div class="ibr-field year">
          <label class="ibr-label">Year</label>
          <input type="number" class="ibr-input import-year" value="${b.year || new Date().getFullYear()}" />
        </div>
        <div class="ibr-field outcome">
          <label class="ibr-label">Outcome</label>
          <select class="ibr-select import-outcome">
